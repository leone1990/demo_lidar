...
{
  //std::cout<<"imagePointsHandler"<<std::endl;
  ...
  double transform[6] = { 0.0001,0.0001,0.0001,0.0001,0.0001,0.0001};
  if (imuPointerLast >= 0)
  {
  ...
  for (int iterCount = 0; iterCount < iterNum; iterCount++)
  {
    ...
    for (int i = 0; i < ipRelationsNum; i++)
    {
      ...
      //std::cout<<"transform[0]:"<<transform[0]<<",transform[1]:"<<transform[1]<<",transform[2]:"<<transform[2]<<std::endl;
      ...
      if (fabs(ipr.v) < 0.5)
      {
            ...
            fabs(y2) < 2 * meanValueWithDepthRec / 10000)
        {
          ...
          //std::cout<<"ipr2.x:"<<ipr2.x<<"ipr2.y:"<<ipr2.y<<std::endl;
    ...
    if (ipRelations2Num > 10)
    {
      ...
      for (int i = 0; i < ipRelations2Num; i++)
      {
        ...
        //std::cout<<"ipr2.x:"<<ipr2.x<<",ipr2.y:"<<ipr2.y<<",ipr2.z:"<<ipr2.z<<std::endl;
      ...
      //}
      //std::cout<<"matX.at<float>(0, 0):"<<matX.at<float>(0, 0)<<",matX.at<float>(1, 0):"<<matX.at<float>(1, 0)<<",matX.at<float>(2, 0):"<<matX.at<float>(2, 0)<<std::endl;
  ...
  double rx, ry, rz;
  //std::cout<<"transformSum[0]:"<<transformSum[0]<<",transformSum[1]:"<<transformSum[1]<<",transformSum[2]:"<<transformSum[2]<<std::endl;
  //std::cout<<"transform[0]:"<<transform[0]<<",transform[1]:"<<transform[1]<<",transform[2]:"<<transform[2]<<std::endl;
  ...
  //std::cout<<"geoQuat.x:"<<geoQuat.x<<",geoQuat.y:"<<geoQuat.y<<",geoQuat.z:"<<geoQuat.z<<",geoQuat.w:"<<geoQuat.w<<std::endl;
...
{
  //std::cout<<"depthCloudHandler"<<std::endl;
